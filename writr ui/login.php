...
//	$user_email =  trim(strip_tags($_POST['email']));
	...
	$validate_user_information = mysqli_query($con,"select * from `users` where `username` = '".mysqli_real_escape_string($con,$user_name)."' and `password` = '".mysqli_real_escape_string($con,$encrypted_md5_password)."'");
	...
	{
        $fetch = mysqli_fetch_assoc($validate_user_information);
        $id = $fetch['id'];
